...
### 1.5 Formal MDP definition

We formalise `CoinNavigationEnv` as a fully observed, deterministic, finite-horizon MDP built on the same state and action primitives as the base navigation task, in which the coin functions as an *instructed* high-reward subgoal rather than as an addition to the environmental state or reward.

**State, action, and transition.** Fix a grid of side length *N* with wall/goal/coin structure given by *G* : [*N*]² → {wall, open, goal, coin} (§1.3). The state space is the set of open interior coordinates *S* = {(*x*, *y*) ∈ [*N*]² : *G*(*x*, *y*) ≠ wall} — identical in form to the base task's state space (§1.1) — and the action space is *A* = {LEFT, RIGHT, UP, DOWN}, indexed 0–3 (the optional QUIT action, when enabled, terminates the episode immediately and is excluded below). Transitions are deterministic: *T*(*s*′ | *s*, *a*) = 𝟙[*s*′ = *f*(*s*, *a*)], where *f*(*s*, *a*) returns the cell adjacent to *s* in direction *a* if that cell is not a wall, and *s* itself otherwise — a wall collision is a self-loop that consumes a time step rather than an error (§1.2). Crucially, the coin cell is traversable both before and after collection (it is a MiniGrid `Ball` explicitly permitted to be overlapped; §1.3), so *T* never depends on whether the coin has already been visited: position alone is a sufficient statistic for the dynamics, and no state augmentation is required to specify *T*. The agent observes *s*<sub>t</sub> directly via the full text rendering of §2.1 (there is no partial observability in this formulation), so the interaction is a fully observed MDP.

**Environmental reward.** The environmental reward is exactly that of the base navigation MDP: *r*(*s*, *a*, *s*′) = 1 − 0.9 · (*t*/*T*) if *s*′ = *s*<sub>goal</sub>, and 0 otherwise, where *t* is the step index at which *s*′ is reached and *T* is the episode's step budget (§6.2). It makes no reference to the coin cell *p*: entering, or repeatedly re-entering, *p* yields zero environmental reward, and episode termination is governed solely by contact with *s*<sub>goal</sub>. As with the base task, *r* is time-indexed and thus non-stationary, consistent with treating the episode as finite-horizon and undiscounted (*γ* = 1) with the time penalty substituting for discounting. This is the formal statement of §1.3's "reward-irrelevant object": as an MDP, (*S*, *A*, *T*, *r*, *γ*) is identical for `CoinNavigationEnv` and `Simple2DNavigationEnv` — the coin cell is, environmentally, just another open cell.

**Coin as an instructed high-reward subgoal.** The coin-directed task is not obtained by modifying *r*; it is imposed externally by a natural-language instruction *φ*, drawn from the objective conditions of §4.2. We formalise the effect of *φ* the same way the subgoal-inference algorithms of §9 treat it when working backward from behaviour: as an *assumed* high-reward, non-terminal target. Define a one-off subgoal reward *r*<sub>coin</sub>(*s*, *a*, *s*′) = 𝟙[*s*′ = *p*] over the same state space *S*. Because the instructed behaviour is sequential — collect the coin, *then* reach the goal — *r*<sub>coin</sub> and *r* are not summed into a single stationary reward; instead they define two ordered, single-target planning problems sharing (*S*, *A*, *T*, *γ*): *M*₁ = (*S*, *A*, *T*, *r*<sub>coin</sub>, *γ*) is solved first, and once *p* is reached the target switches to *M*₂ = (*S*, *A*, *T*, *r*, *γ*). This is precisely how the two-phase oracle `CoinAStarAgent` is implemented (§3.3: it plans toward the coin while uncollected, then toward the goal), and precisely how Bayesian inverse planning (§9.3) scores a candidate subgoal *c* — splitting each trajectory at its first visit to *c* and evaluating the two segments under separate target-directed policies — without ever introducing a collected/uncollected state variable. The reward-over-cells algorithms of §9 (BIRL, MaxEnt IRL) operationalise the same idea in reverse: they treat the observed trajectory as (softly) optimal for an unknown, stationary per-cell reward function over *S* and predict the coin's location as the non-terminal cell of highest inferred reward — recovering *p* without the environment's own state representation ever having needed to carry a coin-status component.

For `TwoCoinNavigationEnv`, the same construction extends to two candidate subgoal cells *p*₁, *p*₂, with the instructed condition determining which ordered or unordered combination of {*M*<sub>coin,1</sub>, *M*<sub>coin,2</sub>, *M*<sub>goal</sub>} constitutes compliant behaviour (either coin, or both, before the goal; §4.2).

**Instructed objective as a task specification external to *r*.** *φ* leaves (*S*, *A*, *T*, *r*, *γ*) unchanged; it defines a success functional *U*<sub>φ</sub> over completed trajectories that is external to the MDP — e.g. *U*<sub>control</sub>(*τ*) = 𝟙[*p* visited in *τ*] · 𝟙[*s*<sub>T</sub> = *s*<sub>goal</sub>] for the coin-directed condition, *U*<sub>avoid</sub>(*τ*) = 𝟙[*p* not visited in *τ*] · 𝟙[*s*<sub>T</sub> = *s*<sub>goal</sub>] for the coin-avoidant condition, and analogous disjunctive/conjunctive predicates over {*p*₁, *p*₂} for the two-coin collect-one and collect-all conditions (the *full success* criterion of §8.1). Agents are evaluated against *U*<sub>φ</sub>, not against the cumulative environmental return Σ*r*, even though *r* (via goal contact) still governs episode termination. This separation of the environment's reward function from the agent's instructed objective is the central methodological device of the framework: because *r* never references the coin, any systematic difference in coin-directed behaviour across instruction conditions *φ* is attributable to instruction-following rather than to reward-driven optimisation, and the same underlying MDP can be paired with arbitrarily many — including mutually conflicting — instructed objectives *φ*.
...
**Policies and trajectories.** We write *π*\* for an optimal policy with respect to the relevant objective — the base navigation MDP, or the two-phase instructed task (*S*, *r*<sub>coin</sub>, *r*, *U*<sub>φ</sub>) of §1.5 — and, following convention, take *π*\* to be uniform over optimal actions where several are tied. `AlphaStarAgent` realises *π*\* for the base navigation MDP by A\* search; `CoinAStarAgent` realises the corresponding two-phase optimal policy for the instructed coin-then-goal task, solving *M*₁ then *M*₂ in sequence (§3.3). `RandomAgent` realises the uniform policy *π*<sub>rand</sub>(*a* | *s*) = 1/|*A*|. An LLM agent parameterised by *θ* and instructed with objective *φ* implements a stochastic policy *π*<sub>θ,φ</sub>(*a*<sub>t</sub> | *h*<sub>t</sub>), where *h*<sub>t</sub> is the information available to the model at step *t*: in the stateless base configuration, *h*<sub>t</sub> reduces to the single, fully observed rendering of *s*<sub>t</sub> (§2.1), so *π*<sub>θ,φ</sub> is Markov in *s*<sub>t</sub>; in the history-tracking variant of §3.2, *h*<sub>t</sub> additionally includes the symbolic action/coin-status history, which does not add information the model could not in principle recover from *s*<sub>t</sub> alone (the grid rendering already reflects whether the coin has been collected) but makes the current phase — whether the active target is *M*₁ or *M*₂ — explicit rather than implicit. Sampling is non-degenerate (temperature 0.7, §5.1), so *π*<sub>θ,φ</sub> is a genuine distribution over actions rather than an argmax policy; repeated trajectories on the same grid (§6.3) are therefore independent draws from *π*<sub>θ,φ</sub>, used to estimate its behavioural statistics (§8) rather than a single deterministic rollout. Given a policy *π* and an initial state *s*₀, a trajectory *τ*<sup>π</sup>(*s*₀) = (*s*<sub>i</sub>, *a*<sub>i</sub>)<sup>T</sup><sub>i=0</sub> is generated by executing *π* from *s*₀ under *T*; a trajectory is successful if *s*<sub>T</sub> = *s*<sub>goal</sub> (base task) or additionally satisfies the instructed success predicate *U*<sub>φ</sub> (coin task, §8.1), and otherwise terminates unsuccessfully at the fixed step budget *T*.
...
**Cross-validated hyperparameter check.** Inverse planning's rationality parameter β and surprise's momentum/decay parameters (λ, γ, α) are taken from prior literature fit to human navigation rather than to this task (β = 2 from Baker et al., 2011; λ, γ, α from Buidze et al., 2025), and the task provides no independent validation signal beyond the coin position already used for scoring. To check whether these literature values are actually supported by LLM-generated trajectories — without letting parameter selection and evaluation touch the same held-out data — a stratified 5-fold cross-validation is run separately (`algorithms/tune_params.py`): grids are pooled by layout and split into folds stratified by grid size; within each fold, the candidate value minimising mean Manhattan distance to the true coin position is grid-searched on the training folds only and then applied, unmodified, to score the held-out fold. The mean ± SD of held-out Manhattan distance across folds is the reported, unbiased estimate of how well a data-fit value generalises; a separate grid search over all usable grids gives a full-data point-estimate and loss curve for reference, and is not itself independently validated. Agreement of the selected value across folds is reported as a stability check. This is a diagnostic/validation analysis only — it does not alter the fixed β = 2 or literature-fit λ, γ, α used in the main comparison above.
