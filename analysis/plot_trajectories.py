...
def process_layout(layout_path: Path, out_dir: Path):
    layout = load_layout(layout_path)
    prefix = layout_path.stem[: -len("_layout")]

    traj_paths = sorted(
        layout_path.parent.glob(f"{prefix}_low_traj*.json"),
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.exit("Usage: python plot_trajectories.py <data_folder>")
