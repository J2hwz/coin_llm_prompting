...
def generate_trajectory(
    ...
    # Determine how to initialise the environment state
    if skip_reset:
        # Use the environment exactly as-is — no reset of any kind
        raw_obs = env.unwrapped.gen_obs()
        observation = env.observation(raw_obs)
    elif use_safe_reset:
        # Reset agent position only, keep the existing grid layout
