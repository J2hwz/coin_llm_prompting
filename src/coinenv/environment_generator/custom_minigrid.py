...
class Simple2DNavigationEnv(MiniGridEnv):
    ...
    def __init__(  # This defines the main arguments passed to the object's initial state
        self,
        size=11,
        ...
        **kwargs,  # This captures any other key word arguments that may be passed into the object, which might be used by the larger MiniGridEnv object
    ):
        ...
        self.place_at_dead_ends = place_at_dead_ends
    ...
    def _gen_grid(self, width, height):
        ...
        # Use dead ends as candidate positions if requested and enough exist
        dead_ends = [
            (x, y)
        ...
        self.goal_pos = (
            self.goal_pos_user
            if self.goal_pos_user in candidate_cells
            else random.choice(candidate_cells)
        )
        self.put_obj(Goal(), *self.goal_pos)
        remaining_cells = [c for c in candidate_cells if c != self.goal_pos]

        # Place Agent
        self.agent_pos = (
            self.agent_start_pos_user
            if self.agent_start_pos_user in remaining_cells
            else random.choice(remaining_cells)
        )
        self.agent_dir = (
...
class CoinNavigationEnv(Simple2DNavigationEnv):
    """Simple2DNavigationEnv with coin (Ball) collection mechanics.

    Identical to Simple2DNavigationEnv except:
    - The agent can walk onto Ball cells.
    - Walking onto a Ball cell collects the coin: it is removed from the grid
      and coin_collected is set to True.
    - Each step returns {"coin_collected": bool} in the info dict.

    The coin is not placed by this class — place it externally using put_obj()
    after reset(), as done in get_trajectory_deadend_env.
    """
    ...
    def step(self, action):
        ...
        # Allow movement onto empty cells, overlappable cells, or ball (coin) cells
...
if __name__ == "__main__":
    ...
    low_complexity_env = Simple2DNavigationEnv(
        size=9, complexity=0.2, render_mode="human"
    )
    ...
    high_complexity_env = Simple2DNavigationEnv(
        size=9, complexity=1.0, render_mode="human"
    )
